...
@section('financeiro-content')
    @if ($conta->exists)
        <div class="card shadow-sm mb-3" id="formulario">
            <div class="card-header">
                <h5 class="mb-0">Alterar Lançamento</h5>
            </div>
            <div class="card-body">

                <form action="{{ route('financeiro.lancamentos.pagar', $conta->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                ...
                <div class="mb-3">
                    <div class="row d-flex align-items-end gap-2">
                        <div class="col-auto">
                            <form action="{{ route('financeiro.lancamentos.cancelar-recebimento', $conta->id) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
    ...
    <div class="card shadow-sm" id="formulario">
        ...
        <div class="card-body">
            ...
            <form
                action="{{ $conta->exists ? route('lancamentos.update', $conta->id) : route('lancamentos.store') }}"
                method="POST">
                @csrf
                @if ($conta->exists)
                    @method('PUT')
